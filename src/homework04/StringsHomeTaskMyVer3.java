...
public class StringsHomeTaskMyVer3 {
     ...
     * @return возвращаемая строка
     */
    private String toStringDifferentGender(long number, boolean isFemale) {
        String sign = "";
        String str = null;
        ...
        int thousandsDegree = (int) (Math.ceil(length.lengthOfNumber(number) / 3.));
        int[] arrOfNumberByThousands = new int[thousandsDegree];
        long cellResult = number;

        for (int i = arrOfNumberByThousands.length - 1; i >= 0; i--) {
            arrOfNumberByThousands[i] =(int) (cellResult % 1000);
            cellResult = number / 1000;
        }

        str = sign + arrayToStringWithAddWords(arrOfNumberByThousands, isFemale);
    ...
    public String toString(double number) {
        if (number < -999_999_999_999.99 || number > 999_999_999_999.99) {
            return "Bad data";
        }
        int fractionalPart;
        long numberInt;
     ...
     * @return возвращаемая строка
     */
    private String additionalWords(long number, String[] words) {
     ...
     *
     * @param array    массив чисел для преобразования
     * @param isFemale мужской/женский род
     * @return возвращаемая строка
     */
    private String arrayToStringWithAddWords(int[] array, boolean isFemale) {
        int arrLength = array.length;
        String numberStr;
        if (arrLength <= 0) {
            numberStr = "bad number";
        }

        switch (arrLength) {
            case 1:
                numberStr = nullToNineHundredNinetyNine(array[0],
                        isFemale);
                break;
            case 2:
                numberStr = nullToNineHundredNinetyNine(array[0], true)
                        + additionalWords(array[0], thousands) + " "
                        + nullToNineHundredNinetyNine(array[1], isFemale);
                break;
            case 3:
                numberStr = nullToNineHundredNinetyNine(array[0], false)
                        + additionalWords(array[0], millions) + " " +
                        nullToNineHundredNinetyNine(array[1], true)
                        + additionalWords(array[1], thousands) + " "
                        + nullToNineHundredNinetyNine(array[2], isFemale);
                break;
            case 4:
                numberStr = nullToNineHundredNinetyNine(array[0], false)
                        + additionalWords(array[0], milliards) + " " +
                        nullToNineHundredNinetyNine(array[1], false)
                        + additionalWords(array[1], millions) + " " +
                        nullToNineHundredNinetyNine(array[2], true)
                        + additionalWords(array[2], thousands) + " "
                        + nullToNineHundredNinetyNine(array[3], isFemale);
                break;
            default:
                numberStr = "It's not work yet";
        }
